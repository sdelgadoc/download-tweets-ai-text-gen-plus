...
def download_account_tweets(username=None,
    ...
    assert label != "", "Enter your environment name into the keys.py file, under \"label\""

    # Validate that limit is a multiple of 100; set total number of tweets
    if limit:
        assert limit % 100 == 0, "`limit` must be a multiple of 100."

        pbar = tqdm(range(limit), desc="Oldest Tweet from " + str(timeframe) + " onwards")
    ...
    if limit is not None:
        cursor = tweepy.Cursor(api.search_full_archive,
                               label=label,
                               query = "from:" + username,
                               fromDate=timeframe).items(limit)
    else:
        cursor = tweepy.Cursor(api.search_full_archive,
                               label=label,
                               query = "from:" + username,
                               fromDate=timeframe).items()
